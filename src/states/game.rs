...
impl GameState {
    ...
    pub fn input(&mut self, rh: &RaylibHandle) {
        ...
        if rh.is_key_down(KeyboardKey::KEY_LEFT) || rh.is_key_pressed(KeyboardKey::KEY_J) {
            self.actions.insert(Action::Command(Command::RotateLeft));
        }

        if rh.is_key_down(KeyboardKey::KEY_RIGHT) || rh.is_key_pressed(KeyboardKey::KEY_L) {
            self.actions.insert(Action::Command(Command::RotateRight));
        }

        if rh.is_key_down(KeyboardKey::KEY_UP) || rh.is_key_pressed(KeyboardKey::KEY_I) {
            self.actions.insert(Action::Command(Command::Accelerate));
        }

        if rh.is_key_down(KeyboardKey::KEY_DOWN) || rh.is_key_pressed(KeyboardKey::KEY_K) {
